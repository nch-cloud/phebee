...
def generate_simplified_ttl(evidence_records: List[dict], subject_map: Dict[Tuple[str, str], str]) -> str:
    """Generate simplified TTL for Neptune from evidence records"""
    ...
    for record in evidence_records:
        ...
        # Encounter and Note (if present)
        note_context = record.get('note_context', {})
        if note_context.get('encounter_id') and note_context.get('note_id'):
            encounter_id = note_context['encounter_id']
            note_id = note_context['note_id']
            
            encounter_iri = f"{subject_iri}/encounter/{encounter_id}"
            note_iri = f"{encounter_iri}/note/{note_id}"
            
            if encounter_id not in encounters_seen:
                ttl_lines.extend([
                    f"{encounter_iri} rdf:type phebee:Encounter ;",
                    f"    phebee:hasSubject {subject_iri} ."
                ])
                encounters_seen.add(encounter_id)
            
            if note_id not in notes_seen:
                ttl_lines.extend([
                    f"{note_iri} rdf:type phebee:ClinicalNote ;",
                    f"    phebee:hasEncounter {encounter_iri} ."
                ])
                notes_seen.add(note_id)
        
        # TermLink (simplified)
        reference_id = record.get('reference_id')
        if reference_id:
            # Create deterministic termlink ID
            termlink_hash = hash((subject_id, reference_id, str(record.get('qualifiers', []))))
            termlink_id = f"termlink_{abs(termlink_hash)}"
            termlink_iri = f"{subject_iri}/termlink/{termlink_id}"
            
            if termlink_id not in termlinks_seen:
                ttl_lines.extend([
                    f"{termlink_iri} rdf:type phebee:TermLink ;",
                    f"    phebee:sourceNode {subject_iri} ;",
                    f"    phebee:hasTerm <{reference_id}> ."
                ])
                
                # Add qualifiers
                for qualifier in record.get('qualifiers', []):
                    if qualifier.get('qualifier_value') == '1':  # Only positive qualifiers
                        qual_type = qualifier.get('qualifier_type')
                        if qual_type:
                            ttl_lines.append(f"{termlink_iri} phebee:hasQualifyingTerm phebee:{qual_type} .")
                
                termlinks_seen.add(termlink_id)
    
    return "\n".join(ttl_lines)

def generate_simplified_ttl_from_rows(rows, subject_map):
    """Generate simplified TTL from Spark Row objects"""
    ttl_lines = [
    ...
    for row in rows:
        subject_iri = f"phebee:subject_{row.subject_id}"
        term_iri = row.term_iri.replace("http://purl.obolibrary.org/obo/", "hp:")
        
        ttl_lines.append(f"{subject_iri} phebee:hasEvidence [")
        ttl_lines.append(f"    phebee:forTerm {term_iri} ;")
        ttl_lines.append(f"    phebee:evidenceType \"{row.evidence_type}\" ;")
        if row.clinical_note_id:
            ttl_lines.append(f"    phebee:clinicalNoteId \"{row.clinical_note_id}\" ;")
        ttl_lines.append(f"    phebee:runId \"{row.run_id}\"")
        ttl_lines.append("] .")
        ttl_lines.append("")
