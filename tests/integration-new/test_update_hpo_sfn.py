"""
Integration tests for UpdateHPOSFN State Machine.

Tests the HPO (Human Phenotype Ontology) update workflow which:
1. Downloads latest HPO release from GitHub
...
def update_hpo_sfn_arn(cloudformation_stack):
    """Get the UpdateHPOSFN state machine ARN from CloudFormation outputs."""
    ...
    # Find UpdateHPOSFNArn output
    for output in outputs:
        if output['OutputKey'] == 'UpdateHPOSFNArn':
            return output['OutputValue']

    pytest.fail(f"UpdateHPOSFNArn output not found in stack {cloudformation_stack}")
...
def test_update_hpo_execution_succeeds(run_hpo_update_once, update_hpo_sfn_arn):
    """Test that the UpdateHPOSFN state machine executes successfully."""
    sfn_client = get_client("stepfunctions")
    ...
    # If downloaded=False: output contains 'hpo' with Payload.downloaded=False
    # If downloaded=True: output contains EventBridge response after full workflow
    assert 'hpo' in output or 'FailedEntryCount' in output, \
        "Output should contain either HPO download info or EventBridge response"

    # If HPO info present, check if download occurred
    if 'hpo' in output:
        hpo_payload = output['hpo'].get('Payload', {})
        print(f"[TEST] HPO Payload: downloaded={hpo_payload.get('downloaded')}, version={hpo_payload.get('version')}")
        assert 'downloaded' in hpo_payload, "HPO payload should indicate if download occurred"


def test_hpo_dynamodb_timestamp_updated(run_hpo_update_once, cloudformation_stack):
    ...
    assert response['Count'] > 0, "DynamoDB should have SOURCE~hpo records"
...
def test_hpo_hierarchy_table_populated(run_hpo_update_once, query_athena, cloudformation_stack):
    """Test that ontology_hierarchy Iceberg table is populated with HPO terms."""
    cf_client = get_client("cloudformation")
    ...
    assert term_count > 10000, \
        f"Expected more than 10,000 HPO terms, but found {term_count}"

    print(f"\n[TEST] Ontology hierarchy table contains {term_count} HPO terms")


def test_hpo_hierarchy_has_required_columns(run_hpo_update_once, query_athena, cloudformation_stack):
    ...
    assert len(results) > 0, "Should return at least one HPO term"
...
def test_hpo_idempotency_second_run_skips(update_hpo_sfn_arn, run_hpo_update_once):
    """
    Test that running HPO update again immediately skips download when version unchanged.
    ...
    execution_name = f"test-hpo-idempotent-{int(time.time())}"
    ...
    while True:
        ...
        if status == 'SUCCEEDED':
            ...
            assert len(install_states) == 0, \
                "Second execution should skip Install HPO step when version unchanged"
