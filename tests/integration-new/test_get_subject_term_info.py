...
def test_get_subject_term_info_no_qualifiers_success(
    ...
    # Wait for analytical table to be updated
    time.sleep(10)
...
def test_get_subject_term_info_qualifiers_mismatch(
    ...
    # Wait for analytical table update
    time.sleep(10)
...
def test_get_subject_term_info_multiple_termlinks_same_term(
    ...
    # Wait for both termlinks to be created
    time.sleep(10)
...
def test_get_subject_term_info_after_evidence_deletion(
    ...
    # Wait for analytical table to update (up to 60 seconds)
    time.sleep(5)  # Initial wait for processing
    for _ in range(11):  # Try for up to 60 seconds total
        result = invoke_get_subject_term_info(subject_uuid, term_iri)
        if result["statusCode"] == 200:
            body = json.loads(result["body"])
            if body["evidence_count"] == 2:
                break
        time.sleep(5)
    ...
    # Wait for term link removal (up to 60 seconds)
    time.sleep(5)
    for _ in range(11):
        result = invoke_get_subject_term_info(subject_uuid, term_iri)
...
def test_get_subject_term_info_concurrent(
    ...
    # Wait for all analytical tables to update
    time.sleep(10)
