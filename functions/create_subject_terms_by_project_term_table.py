...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    """
    Custom resource handler for creating Iceberg subject-terms table partitioned by (project_id, term_id).
    Optimized for term-filtered queries and all-subjects-in-project queries.
    """
    try:
        ...
        logger.error(f"Error in create_subject_terms_by_project_term_table: {str(e)}")
        send_response(event, context, 'FAILED', {'Error': str(e)})

def create_table(database: str, table: str, s3_location: str, query_results_s3: str):
    """Create the Iceberg subject-terms table partitioned by (project_id, term_id)."""
    ...
    PARTITIONED BY (project_id, term_id)
    LOCATION '{s3_location}'
    TBLPROPERTIES (
        'table_type'='ICEBERG',
        'format'='parquet',
        'write.parquet.compression-codec'='snappy',
        'description'='Subject-term associations partitioned by (project_id, term_id) for term-filtered queries'
    )
    """

    logger.info(f"Creating subject-terms-by-project-term table with SQL: {create_table_sql}")
