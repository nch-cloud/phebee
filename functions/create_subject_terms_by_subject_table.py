...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    """
    Custom resource handler for creating Iceberg subject-terms table partitioned by subject_id.
    ...
    try:
        ...
        logger.error(f"Error in create_subject_terms_by_subject_table: {str(e)}")
        send_response(event, context, 'FAILED', {'Error': str(e)})

def create_table(database: str, table: str, s3_location: str, query_results_s3: str):
    """Create the Iceberg subject-terms table partitioned by subject_id."""
    ...
    logger.info(f"Creating subject-terms-by-subject table with SQL: {create_table_sql}")
